...
\twoauthors
  ...
  {Haowen Chen\sthanks{Corresponding author: Haowen Chen (hwchen@hnu.edu.cn). Funding and compliance statement to be added before submission.}}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.96\textwidth]{figures/ro_pipeline.jpg}
...
RO should be treated as a validated inference wrapper, not a universal replacement for self-consistency. Reports should state the generation budget, preserve question-level pairing, retain parser failures, include randomized-map controls and compare permutation-aware baselines. The repository archives protocols, manifests, raw records and full-precision analyses; remote and local suites reported 25 and 30 passing tests. Future work should test larger option spaces, non-cyclic transformations and unified matched-compute baselines.
